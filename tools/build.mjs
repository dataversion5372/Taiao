...
// ---- QuestScript sources ---------------------------------------------------
// Inline every scripts/**/*.qs quest/interaction script into a global
// SCRIPTS_SRC map, written to js/questscript/qs-scripts-gen.js, so the VM
// (js/questscript/) can parse them at boot with no fetch. Regenerated each
// build; the file is gitignored. Keyed by path-below-scripts sans extension
// (e.g. "quests/bobs_axe").
function collectQs(dir, base, out) {
  let ents;
  try { ents = readdirSync(dir, { withFileTypes: true }); } catch (e) { return; }
  for (const ent of ents) {
    const full = path.join(dir, ent.name);
    if (ent.isDirectory()) collectQs(full, base, out);
    else if (ent.name.endsWith(".qs")) {
      const key = path.relative(base, full).replace(/\\/g, "/").replace(/\.qs$/, "");
      out[key] = readFileSync(full, "utf8");
    }
  }
}
const qsScripts = {};
collectQs(path.join(ROOT, "scripts"), path.join(ROOT, "scripts"), qsScripts);
writeFileSync(
  path.join(ROOT, "js/questscript/qs-scripts-gen.js"),
  `// AUTO-GENERATED by tools/build.mjs — do not edit. Inlined scripts/**/*.qs.\n`
  + `"use strict";\nvar SCRIPTS_SRC = ${JSON.stringify(qsScripts, null, 2)};\n`
);
...
console.log(`\nQuestScript: inlined ${Object.keys(qsScripts).length} .qs script(s) -> js/questscript/qs-scripts-gen.js`);
console.log(`Bundled ${bundleFiles.length} files from tools/bundle.list`);
