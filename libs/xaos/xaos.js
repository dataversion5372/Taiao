...
/* == Taiao vendoring note ==================================================
 * Vendored verbatim from https://github.com/xaos-project/XaoSjs (js/xaos.js,
 * master) for the Tūhura Isle departure cinematic, under GPL-3.0-or-later
 * (same license as Taiao). Three marked patches, each tagged [taiao-patch]:
 *   1. xaos.zoom() returns the ZoomContext so a cinematic can autopilot
 *      the zoom (mutate fractal.region, call drawFractal(false)) instead
 *      of relying on mouse input.
 *   2. the per-frame "N fps" console.log is removed.
 *   3. the trailing auto-start against #canvas is removed (Taiao has no
 *      #canvas element; the game attaches XaoS to its own overlay).
 * ========================================================================== */
...
xaos.zoom = (function() {
    ...
    ZoomContext.prototype.updateFPS = function() {
        ...
        // [taiao-patch 2] per-frame fps console.log removed
    };
    ...
    return function(canvas, fractal) {
        ...
        return zoomer; // [taiao-patch 1] expose the zoomer for programmatic (cinematic) drive
    }
}());
...
// [taiao-patch 3] auto-start removed — Taiao attaches XaoS to its own overlay canvas